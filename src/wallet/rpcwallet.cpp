...
static UniValue getnewaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "\nReturns a new Btcnano address for receiving payments.\n"
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new btcnano address\n"
            "\nExamples:\n" +
...
static UniValue getaccountaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Btcnano address for receiving "
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account btcnano address\n"
            "\nExamples:\n" +
...
static UniValue getrawchangeaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Btcnano address, for receiving change.\n"
...
static UniValue setaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The btcnano address to "
...
static UniValue getaddressesbyaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a btcnano address associated with "
            "the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
static UniValue listaddressgroupings(const Config &config,
    ...
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The btcnano address\n"
...
static UniValue sendfrom(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
            ...
            "reflect\n"
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The btcnano address "
            "to send funds to.\n"
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The btcnano "
            ...
            "btcnanos than you enter in their corresponding amount field.\n"
    ...
    for (const std::string &name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Btcnano address: ") +
                                   name_);
        }
...
static UniValue addmultisigaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "Each key is a Btcnano address or hex-encoded public key.\n"
            ...
            "2. \"keys\"         (string, required) A json array of btcnano "
            "addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) btcnano address or hex-encoded "
            "public key\n"
            "       ...,\n"
            "     ]\n"
...
static UniValue listtransactions(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The btcnano address of "
...
static UniValue gettransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "default account.\n"
            "      \"address\" : \"address\",          (string) The btcnano "
...
static UniValue walletpassphrase(const Config &config,
    ...
    if (pwalletMain->IsCrypted() &&
        ...
        throw std::runtime_error(
            ...
            "private keys such as sending btcnanos\n"
            "\nArguments:\n"
...
static UniValue encryptwallet(const Config &config,
    ...
    StartShutdown();
    return "wallet encrypted; Btcnano server stopping, restart to run with "
...
static UniValue lockunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "selection, when spending btcnanos.\n"
...
static UniValue listunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of btcnano addresses to "
            "filter\n"
            "    [\n"
            "      \"address\"   (string) btcnano address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the btcnano address\n"
...
static UniValue fundrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address) The btcnano address to receive the change\n"
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid btcnano address");
                }
