#!/usr/bin/env python3
# Copyright (c) 2014-2016 The Btcnano Core developers
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "btcnano.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_btcnanod_start(process, url, i):
    '''
    Wait for btcnanod to start. This means that RPC is accessible and fully initialized.
    Raise an exception if btcnanod exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception(
                'btcnanod exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        # Create cache directories, run btcnanods:
        for i in range(MAX_NODES):
            datadir = initialize_datadir(cachedir, i)
            args = [os.getenv("BTCNANOD", "btcnanod"), "-server",
                    "-keypool=1", "-datadir=" + datadir, "-discover=0"]
            if i > 0:
                args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
            btcnanod_processes[i] = subprocess.Popen(args)
            logger.debug("Creating data directories from cached datadir")
            wait_for_btcnanod_start(btcnanod_processes[i], rpc_url(i), i)
    ...
    for i in range(num_nodes):
        ...
        # Overwrite port/rpcport in btcnano.conf
...
def locate_btcnanod_binary():
    """
    Find btcnanod binary if possible.
    """
    btcnanod_binary = os.getenv("BTCNANOD", DEFAULT_BTCNANOD)
    if os.path.exists(btcnanod_binary):
        return btcnanod_binary

    if os.path.exists(os.path.join('src', DEFAULT_BTCNANOD)):
        btcnanod_binary = os.path.abspath(
            os.path.join('src', DEFAULT_BTCNANOD))
    elif btcnanod_binary == 'btcnanod' or not os.path.exists(btcnanod_binary):
        # If BTCNANOD was specified and exists, use it, otherwise look for source.
        # get_srcdir() already returns an absolute path
        src_dir_cand = get_srcdir(sys.argv[0])
        if src_dir_cand and os.path.exists(
                os.path.join(src_dir_cand, 'src', DEFAULT_BTCNANOD)):
            btcnanod_binary = os.path.join(
                src_dir_cand, 'src', DEFAULT_BTCNANOD)
        else:
            sys.stderr.write("Unable to locate btcnanod for this test.\n")
            sys.exit(1)
    return btcnanod_binary
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr_checker=None):
    """
    Start a btcnanod and return RPC connection to it.
    If stderr_checker is provided, it must be an OutputChecker.
    Its output_file_obj will be connected to the stderr of the btcnanod process.
    """
    datadir = os.path.join(dirname, "node" + str(i))
    if binary is None:
        binary = locate_btcnanod_binary()
    ...
    if stderr_checker:
        ...
        btcnanod_processes[i] = subprocess.Popen(args)
    logger.debug(
        "initialize_chain: btcnanod started, waiting for RPC to come up")
    url = rpc_url(i, rpchost)
    wait_for_btcnanod_start(btcnanod_processes[i], url, i)
    logger.debug("initialize_chain: RPC successfully started")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr_checkers=None):
    """
    Start multiple btcnanods, return RPC connections to them
...
def stop_node(node, i):
    ...
    return_code = btcnanod_processes[i].wait(
        timeout=BTCNANOD_PROC_WAIT_TIMEOUT)
    assert_equal(return_code, 0)
    del btcnanod_processes[i]


def stop_nodes(nodes):
    for i, node in enumerate(nodes):
        stop_node(node, i)
    assert not btcnanod_processes.values()  # All connections must be gone now
