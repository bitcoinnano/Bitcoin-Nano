...
const char *const BTCNANO_CONF_FILENAME = "btcnano.conf";
const char *const BTCNANO_PID_FILENAME = "btcnanod.pid";
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Btcnano
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Btcnano
// Mac: ~/Library/Application Support/Btcnano
// Unix: ~/.btcnano
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Btcnano";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Btcnano";
#else
    // Unix
    return pathRet / ".btcnano";
#endif
#endif
}
...
boost::filesystem::path GetPidFile() {
    boost::filesystem::path pathPidFile(GetArg("-pid", BTCNANO_PID_FILENAME));
...
std::string CopyrightHolders(const std::string &strPrefix) {
    ...
    // Check for untranslated substitution to make sure Btcnano Core copyright
    // is not removed by accident.
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION)
            .find("Btcnano Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Btcnano Core developers";
    }
