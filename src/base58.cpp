...
class CBtcnanoAddressVisitor : public boost::static_visitor<bool> {
private:
    CBtcnanoAddress *addr;

public:
    CBtcnanoAddressVisitor(CBtcnanoAddress *addrIn) : addr(addrIn) {}
...
bool CBtcnanoAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CBtcnanoAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CBtcnanoAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CBtcnanoAddressVisitor(this), dest);
}

bool CBtcnanoAddress::IsValid() const {
    return IsValid(Params());
}

bool CBtcnanoAddress::IsValid(const CChainParams &params) const {
...
CTxDestination CBtcnanoAddress::Get() const {
...
void CBtcnanoSecret::SetKey(const CKey &vchSecret) {
...
bool CBtcnanoSecret::IsValid() const {
...
bool CBtcnanoSecret::SetString(const char *pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CBtcnanoSecret::SetString(const std::string &strSecret) {
    return SetString(strSecret.c_str());
}

std::string EncodeDestination(const CTxDestination &dest) {
    CBtcnanoAddress addr(dest);
    return addr.IsValid() ? addr.ToString() : "";
}

CTxDestination DecodeDestination(const std::string &str) {
    return CBtcnanoAddress(str).Get();
}

bool IsValidDestinationString(const std::string &str,
                              const CChainParams &params) {
    return CBtcnanoAddress(str).IsValid(params);
}

bool IsValidDestinationString(const std::string &str) {
    return CBtcnanoAddress(str).IsValid();
}
