...
*Setup instructions for a Gitian build of Btcnano Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Btcnano
...
- [Building Btcnano Core](#building-btcnano-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-btcnano-host/README.md).
...
Clone the git repositories for btcnano and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/btcnano/btcnano
...
in the btcnano repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Btcnano Core
----------------

To build Btcnano Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the btcnano repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/btcnano/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/btcnano/btcnano
...
For example:
```bash
URL=https://github.com/laanwj/btcnano.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit btcnano=${COMMIT} --url btcnano=${URL} ../btcnano/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit btcnano=${COMMIT} --url btcnano=${URL} ../btcnano/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit btcnano=${COMMIT} --url btcnano=${URL} ../btcnano/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the btcnano git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../btcnano/contrib/gitian-descriptors/*|sort|uniq )
...
BTNPATH=/some/root/path/btcnano
SIGPATH=/some/root/path/btcnano-detached-sigs

./bin/gbuild --url btcnano=${BTNPATH},signature=${SIGPATH} ../btcnano/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/btcnano-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/btcnano-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/btcnano-osx-build.assert
```
...
[btcnano-core/gitian.sigs](https://github.com/btcnano-core/gitian.sigs/) repository, or if that's not possible create a pull
