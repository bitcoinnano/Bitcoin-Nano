...
#include "btcnanoaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent) {
    ...
    widget->setPlaceholderText(
        QObject::tr("Enter a Btcnano address (e.g. %1)")
            .arg(QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new BtcnanoAddressEntryValidator(parent));
    widget->setCheckValidator(new BtcnanoAddressCheckValidator(parent));
}
...
bool parseBtcnanoURI(const QUrl &uri, SendCoinsRecipient *out) {
    ...
    for (QList<QPair<QString, QString>>::iterator i = items.begin();
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BtcnanoUnits::parse(BtcnanoUnits::BTN, i->second,
                                         &rv.amount)) {
                    return false;
                }
            }
...
bool parseBtcnanoURI(QString uri, SendCoinsRecipient *out) {
    ...
    return parseBtcnanoURI(uriInstance, out);
}

QString formatBtcnanoURI(const SendCoinsRecipient &info) {
    ...
    if (info.amount) {
        ret +=
            QString("?amount=%1")
                .arg(BtcnanoUnits::format(BtcnanoUnits::BTN, info.amount, false,
                                          BtcnanoUnits::separatorNever));
        paramCount++;
    }
...
static boost::filesystem::path StartupShortcutPath() {
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Btcnano.lnk";
    // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Btcnano (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) /
           strprintf("Btcnano (%s).lnk", chain);
}

bool GetStartOnSystemStartup() {
    // check for Btcnano*.lnk
...
static boost::filesystem::path GetAutostartFilePath() {
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "btcnano.desktop";
    return GetAutostartDir() / strprintf("btcnano-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart) {
    if (!fAutoStart)
        ...
        if (!optionFile.good()) return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a btcnano.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Btcnano\n";
        else
            optionFile << strprintf("Name=Btcnano (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list,
    ...
    // loop through the list of startup items and try to find the btcnano app
...
bool GetStartOnSystemStartup() {
    CFURLRef btcnanoAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
