...
const int BTCNANO_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
// BIP70 payment protocol messages
const char *BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char *BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char *BIP71_MIMETYPE_PAYMENT = "application/btcnanocash-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/btcnanocash-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST =
    "application/btcnanocash-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char *argv[]) {
    for (int i = 1; i < argc; i++) {
        ...
        // If the btcnanocash: URI contains a payment request, we are not able
        ...
        if (arg.startsWith(GUIUtil::URI_SCHEME + ":",
                           Qt::CaseInsensitive)) // btcnanocash: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseBtcnanoURI(arg, &r) && !r.address.isEmpty()) {
...
bool PaymentServer::ipcSendCommandLine() {
    ...
    for (const QString &r : savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(BTCNANO_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BTCNANO_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer)
    ...
    // on Mac: sent when you click btcnanocash: links
...
//
// OSX-specific way of handling btcnanocash: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager() {
    ...
    // netManager is used to fetch paymentrequests given in btcnanocash: URIs
...
void PaymentServer::handleURIOrFile(const QString &s) {
    ...
    if (s.startsWith(GUIUtil::URI_SCHEME + ":", Qt::CaseInsensitive)) {
        ...
        if (uri.hasQueryItem("r")) {
            ...
            if (GUIUtil::parseBtcnanoURI(s, &recipient)) {
                ...
                Q_EMIT message(
                    ...
                    tr("URI cannot be parsed! This can be caused by an invalid "
                       "Btcnano address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
    ...
    for (const std::pair<CScript, CAmount> &sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom btcnano addresses are
            // not supported
        ...
        // Btcnano amounts are stored as (optional) uint64 in the protobuf
        ...
        if (txOut.IsDust(dustRelayFee)) {
            Q_EMIT message(
                ...
                tr("Requested payment amount of %1 is too small (considered "
                   "dust).")
                    .arg(BtcnanoUnits::formatWithUnit(
