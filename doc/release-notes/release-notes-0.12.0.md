Btcnano Core version 0.12.0 is now available from:

  <https://btcnano.org/bin/btcnano-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Btcnano-Qt (on Mac) or
btcnanod/btcnano-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Btcnano Core or other software:
...
pre-0.12 versions of Btcnano Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Btcnano Core version 0.11 or
earlier.
...
ECDSA signatures inside Btcnano transactions now use validation using
[libsecp256k1](https://github.com/btcnano-core/secp256k1) instead of OpenSSL.
...
(https://github.com/btcnano/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Btcnano Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Btcnano Core was to raise the default minimum
relay fee.

Btcnano Core 0.12 will have a strict maximum size on the mempool. The
...
Btcnano Core 0.12 also introduces new default policy limits on the length and
...
Btcnano Core 0.12 nodes. Btcnano Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/btcnano/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Btcnano Core 0.12 does not yet have support for
...
This allows running btcnanod without having to do any manual configuration.
...
Btcnano Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Btcnano Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Btcnano Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Btcnano Core 0.13, and it is at this time undecided whether the
...
Btcnano Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Btcnano Core automatically creates a hidden service to listen on, without
manual configuration. Btcnano Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Btcnano Core is listening, and
...
Btcnanod can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Btcnano
Core to use floating fees. By default, Btcnano Core will use floating
fees.
...
(default: `0.0002` BTN/kB).

At all times, Btcnano Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTN.
Furthermore, Btcnano Core will never create transactions paying less than
...
`btcnano.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/btcnano/btcnano/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/btcnano/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/btcnano/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 29657 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 29657 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /btcnanorpc>
        ProxyPass http://127.0.0.1:9657/
        ProxyPassReverse http://127.0.0.1:9657/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass btcnanod rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< btcnanorpc:<password>
    </Location>
...
Restarting btcnanod will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Btcnano-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new btcnano-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include btcnano-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix btcnano-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of btcnano-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override BTCNANOD and BTCNANOCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [btcnano-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BTCNANOD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libbtcnanoconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "BTN" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/btcnano/).
