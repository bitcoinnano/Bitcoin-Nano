...
###   This script attempts to download the signature file SHA256SUMS.asc from btcnano.org
...
WORKINGDIR="/tmp/btcnano_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://btcnano.org/bin/"
VERSIONPREFIX="btcnano-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'btcnano-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Btcnano Core binary release signing key installed?"
   fi
