...
void SendCoinsDialog::on_sendButton_clicked() {
    ...
    processSendCoinsReturn(
        prepareStatus,
        BtcnanoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
    ...
    for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients()) {
        ...
        QString amount =
            "<b>" + BtcnanoUnits::formatHtmlWithUnit(
    ...
    if (txFee > 0) {
        ...
        questionString.append(BtcnanoUnits::formatHtmlWithUnit(
    ...
    for (BtcnanoUnits::Unit u : BtcnanoUnits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(
                BtcnanoUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(
        tr("Total Amount %1")
            .arg(BtcnanoUnits::formatHtmlWithUnit(
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text) {
    if (model && model->getAddressTableModel()) {
        ...
        if (text.isEmpty()) {
            ...
            // Invalid address
            ui->labelCoinControlChangeLabel->setText(
                tr("Warning: Invalid Btcnano address"));
        } else {
            // Valid address
