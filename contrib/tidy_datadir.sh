#!/bin/bash
# Copyright (c) 2013 The Btcnano Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Btcnano database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Btcnano datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Btcnano datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Btcnano 0.7 datadir."
    ;;
  3)
    echo "Detected Btcnano pre-0.8 datadir."
    ;;
  4)
    echo "Detected Btcnano 0.8 datadir."
    ;;
esac
