 ...
 * experimental new digital currency called Btcnano (https://www.btcnano.org/),
 * which enables instant payments to anyone, anywhere in the world. Btcnano uses
...
bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/btcnano.conf are parsed in qt/btcnano.cpp's
    // main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) &&
                !boost::algorithm::istarts_with(argv[i], "btcnano:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "btcnanod anymore. Use the btcnano-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for btcnanod but not for the GUI so do this
        // here
...
#if HAVE_DECL_DAEMON
            fprintf(stdout, "Btcnano server starting\n");
...
int main(int argc, char *argv[]) {
    ...
    // Connect btcnanod signal handlers
    noui_connect();
