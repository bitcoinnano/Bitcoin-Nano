...
const std::string BtcnanoGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
 ...
 */
const QString BtcnanoGUI::DEFAULT_WALLET = "~Default";

BtcnanoGUI::BtcnanoGUI(const PlatformStyle *_platformStyle,
...
void BtcnanoGUI::createActions() {
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Btcnano address"));
    ...
    signMessageAction->setStatusTip(
        tr("Sign messages with your Btcnano addresses to prove you own them"));
    verifyMessageAction =
        new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                    tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(
        tr("Verify messages to ensure they were signed with specified Btcnano "
           "addresses"));
    ...
    showHelpMessageAction->setStatusTip(
        tr("Show the %1 help message to get a list with possible Btcnano "
...
void BtcnanoGUI::createToolBars() {
    if (walletFrame) {
...
void BtcnanoGUI::setClientModel(ClientModel *_clientModel) {
...
#ifdef ENABLE_WALLET
bool BtcnanoGUI::addWallet(const QString &name, WalletModel *walletModel) {
    if (!walletFrame) return false;
    setWalletActionsEnabled(true);
    return walletFrame->addWallet(name, walletModel);
}

bool BtcnanoGUI::setCurrentWallet(const QString &name) {
    if (!walletFrame) return false;
    return walletFrame->setCurrentWallet(name);
}

void BtcnanoGUI::removeAllWallets() {
...
void BtcnanoGUI::setWalletActionsEnabled(bool enabled) {
...
void BtcnanoGUI::createTrayIcon(const NetworkStyle *networkStyle) {
#ifndef Q_OS_MAC
...
void BtcnanoGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BtcnanoGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason) {
...
void BtcnanoGUI::optionsClicked() {
...
void BtcnanoGUI::showDebugWindow() {
...
void BtcnanoGUI::showDebugWindowActivateConsole() {
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BtcnanoGUI::showHelpMessageClicked() {
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void BtcnanoGUI::openClicked() {
...
void BtcnanoGUI::gotoOverviewPage() {
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BtcnanoGUI::gotoHistoryPage() {
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BtcnanoGUI::gotoReceiveCoinsPage() {
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BtcnanoGUI::gotoSendCoinsPage(QString addr) {
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BtcnanoGUI::gotoSignMessageTab(QString addr) {
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BtcnanoGUI::gotoVerifyMessageTab(QString addr) {
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BtcnanoGUI::updateNetworkState() {
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Btcnano network", "", count) +
...
void BtcnanoGUI::setNumConnections(int count) {
    updateNetworkState();
}

void BtcnanoGUI::setNetworkActive(bool networkActive) {
    updateNetworkState();
}

void BtcnanoGUI::updateHeadersSyncProgressLabel() {
...
void BtcnanoGUI::setNumBlocks(int count, const QDateTime &blockDate,
...
void BtcnanoGUI::message(const QString &title, const QString &message,
                         unsigned int style, bool *ret) {
    // default title
    QString strTitle = tr("Btcnano");
...
void BtcnanoGUI::changeEvent(QEvent *e) {
...
void BtcnanoGUI::closeEvent(QCloseEvent *event) {
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void BtcnanoGUI::showEvent(QShowEvent *event) {
...
#ifdef ENABLE_WALLET
void BtcnanoGUI::incomingTransaction(const QString &date, int unit,
    ...
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n")
                      .arg(BtcnanoUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void BtcnanoGUI::dragEnterEvent(QDragEnterEvent *event) {
    // Accept only URIs
    if (event->mimeData()->hasUrls()) event->acceptProposedAction();
}

void BtcnanoGUI::dropEvent(QDropEvent *event) {
...
bool BtcnanoGUI::eventFilter(QObject *object, QEvent *event) {
...
#ifdef ENABLE_WALLET
bool BtcnanoGUI::handlePaymentRequest(const SendCoinsRecipient &recipient) {
    // URI has to be valid
...
void BtcnanoGUI::setHDStatus(int hdEnabled) {
...
void BtcnanoGUI::setEncryptionStatus(int status) {
    switch (status) {
...
void BtcnanoGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel) return;
...
void BtcnanoGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void BtcnanoGUI::detectShutdown() {
...
void BtcnanoGUI::showProgress(const QString &title, int nProgress) {
    if (nProgress == 0) {
...
void BtcnanoGUI::setTrayIconVisible(bool fHideTrayIcon) {
    if (trayIcon) {
        trayIcon->setVisible(!fHideTrayIcon);
    }
}

void BtcnanoGUI::showModalOverlay() {
    if (modalOverlay &&
        (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BtcnanoGUI *gui, const std::string &message,
...
void BtcnanoGUI::subscribeToCoreSignals() {
...
void BtcnanoGUI::unsubscribeFromCoreSignals() {
...
void BtcnanoGUI::toggleNetworkActive() {
    if (clientModel) {
...
UnitDisplayStatusBarControl::UnitDisplayStatusBarControl(
    ...
    QList<BtcnanoUnits::Unit> units = BtcnanoUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BtcnanoUnits::Unit unit : units) {
        max_width = qMax(max_width, fm.width(BtcnanoUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu() {
    menu = new QMenu(this);
    for (BtcnanoUnits::Unit u : BtcnanoUnits::availableUnits()) {
        QAction *menuAction = new QAction(QString(BtcnanoUnits::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits) {
    setText(BtcnanoUnits::name(newUnits));
}
