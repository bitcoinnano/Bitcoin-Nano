 ...
 * is shared between btcnanod, and btcnano-qt and non-server tools.
 */
...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    strUsage += HelpMessageOpt(
        "-conf=<file>", strprintf(_("Specify configuration file (default: %s)"),
                                  BTCNANO_CONF_FILENAME));
    if (mode == HMM_BTCNANOD) {
#if HAVE_DECL_DAEMON
        strUsage += HelpMessageOpt(
            "-daemon",
    ...
    strUsage += HelpMessageOpt(
        "-pid=<file>",
        strprintf(_("Specify pid file (default: %s)"), BTCNANO_PID_FILENAME));
#endif
    strUsage += HelpMessageOpt(
        "-prune=<n>",
        strprintf(
    ...
    if (mode == HMM_BTCNANO_QT) debugCategories += ", qt";
    strUsage += HelpMessageOpt(
        "-debug=<category>",
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/Btcnano-NANO/btcnano-nano>";
    const std::string URL_WEBSITE = "<https://www.btcnano.org>";
...
/** Sanity checks
 *  Ensure that Btcnano is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single Btcnano process is using the data directory.
...
bool AppInitMain(Config &config, boost::thread_group &threadGroup,
    ...
    LogPrintf("Using config file %s\n",
              GetConfigFile(GetArg("-conf", BTCNANO_CONF_FILENAME)).string());
