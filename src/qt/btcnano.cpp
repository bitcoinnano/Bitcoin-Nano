...
static std::string Translate(const char *psz) {
    return QCoreApplication::translate("btcnano-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase,
    ...
    // Load e.g. btcnano_de.qm (shortcut "de" needs to be defined in
    // btcnano.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. btcnano_de_DE.qm (shortcut "de_DE" needs to be defined in
    // btcnano.qrc)
...
/** Class encapsulating Btcnano Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class BtcnanoCore : public QObject {
    Q_OBJECT
public:
    explicit BtcnanoCore();
...
/** Main Btcnano application object */
class BtcnanoApplication : public QApplication {
    Q_OBJECT
public:
    explicit BtcnanoApplication(int &argc, char **argv);
    ~BtcnanoApplication();
    ...
    /// Get window identifier of QMainWindow (BtcnanoGUI)
...
BtcnanoCore::BtcnanoCore() : QObject() {}

void BtcnanoCore::handleRunawayException(const std::exception *e) {
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

void BtcnanoCore::initialize(Config *cfg) {
    Config &config(*cfg);
    try {
...
BtcnanoApplication::BtcnanoApplication(int &argc, char **argv)
    ...
    // This must be done inside the BtcnanoApplication constructor, or after it,
    // because PlatformStyle::instantiate requires a QApplication.
    std::string platformName;
    platformName = GetArg("-uiplatform", BtcnanoGUI::DEFAULT_UIPLATFORM);
    platformStyle =
...
BtcnanoApplication::~BtcnanoApplication() {
    if (coreThread) {
...
#ifdef ENABLE_WALLET
void BtcnanoApplication::createPaymentServer() {
    paymentServer = new PaymentServer(this);
}
#endif

void BtcnanoApplication::createOptionsModel(bool resetSettings) {
    optionsModel = new OptionsModel(nullptr, resetSettings);
}

void BtcnanoApplication::createWindow(const NetworkStyle *networkStyle) {
    window = new BtcnanoGUI(platformStyle, networkStyle, 0);
...
void BtcnanoApplication::createSplashScreen(const NetworkStyle *networkStyle) {
...
void BtcnanoApplication::startThread() {
    if (coreThread) return;
    coreThread = new QThread(this);
    BtcnanoCore *executor = new BtcnanoCore();
...
void BtcnanoApplication::parameterSetup() {
    InitLogging();
    InitParameterInteraction();
}

void BtcnanoApplication::requestInitialize(Config &config) {
...
void BtcnanoApplication::requestShutdown(Config &config) {
...
void BtcnanoApplication::initializeResult(int retval) {
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BtcnanoGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BtcnanoGUI::DEFAULT_WALLET);
...
void BtcnanoApplication::shutdownResult(int retval) {
    qDebug() << __func__ << ": Shutdown result: " << retval;
    // Exit main loop after shutdown finished.
    quit();
}

void BtcnanoApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(
        0, "Runaway exception",
        BtcnanoGUI::tr("A fatal error occurred. Btcnano can no longer continue "
                       "safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId BtcnanoApplication::getMainWinId() const {
    if (!window) return 0;

    return window->winId();
}

#ifndef BTCNANO_QT_TEST

static void MigrateSettings() {
    assert(!QApplication::applicationName().isEmpty());

    static const QString legacyAppName("Btcnano-Qt"),
#ifdef Q_OS_DARWIN
    ...
    // We only migrate settings if we have Core settings but no Btcnano-NANO
    // settings
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(btcnano);
    Q_INIT_RESOURCE(btcnano_locale);

    BtcnanoApplication app(argc, argv);
    ...
    // Migrate settings from core's/our old GUI settings to Btcnano Nano
    // only if core's exist but Btcnano Nano's doesn't.
    ...
    /// 6. Determine availability of data directory and parse btcnano.conf
    ...
    try {
        ReadConfigFile(GetArg("-conf", BTCNANO_CONF_FILENAME));
    } catch (const std::exception &e) {
        QMessageBox::critical(
            0, QObject::tr(PACKAGE_NAME),
