#!/usr/bin/env python3
# Copyright (c) 2015-2016 The Btcnano Core developers
...
'''
Test plan:
- Start btcnanod's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on btcnanod side:
...
- Create btcnanods that connect to them
- Manipulate the btcnanods using addnode (onetry) an observe effects
...
class ProxyTest(BtcnanoTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: btcnanod's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert(isinstance(cmd, Socks5Command))
            # Note: btcnanod's SOCKS5 implementation only sends atyp
        ...
        if test_onion:
            ...
            node.addnode("btcnanoostk4e4re.onion:9556", "onetry")
            cmd = proxies[2].queue.get()
            assert(isinstance(cmd, Socks5Command))
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"btcnanoostk4e4re.onion")
            assert_equal(cmd.port, 9556)
            if not auth:
                assert_equal(cmd.username, None)
                assert_equal(cmd.password, None)
            rv.append(cmd)
        ...
        node.addnode("node.noumenon:9556", "onetry")
