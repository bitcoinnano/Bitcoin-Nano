...
#include "config/btcnano-config.h" /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update() {
    ...
    QString uri = GUIUtil::formatBtcnanoURI(info);
    ...
    if (info.amount)
        html += "<b>" + tr("Amount") + "</b>: " +
                BtcnanoUnits::formatHtmlWithUnit(model->getDisplayUnit(),
                                                 info.amount) +
                "<br>";
...
void ReceiveRequestDialog::on_btnCopyURI_clicked() {
    GUIUtil::setClipboard(GUIUtil::formatBtcnanoURI(info));
}
