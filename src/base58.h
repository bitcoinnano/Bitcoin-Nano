 ...
 * alphanumeric.
 */
#ifndef BTCNANO_BASE58_H
#define BTCNANO_BASE58_H
...
public:
    ...
    CBtcnanoSecret(const CKey &vchSecret) { SetKey(vchSecret); }
    CBtcnanoSecret() {}
};

template <typename K, int Size, CChainParams::Base58Type Type>
class CBtcnanoExtKeyBase : public CBase58Data {
public:
    ...
    CBtcnanoExtKeyBase(const K &key) { SetKey(key); }

    CBtcnanoExtKeyBase(const std::string &strBase58c) {
...
typedef CBtcnanoExtKeyBase<CExtKey, BIP32_EXTKEY_SIZE,
                           CChainParams::EXT_SECRET_KEY>
    CBtcnanoExtKey;
typedef CBtcnanoExtKeyBase<CExtPubKey, BIP32_EXTKEY_SIZE,
