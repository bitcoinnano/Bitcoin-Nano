...
The RPM spec file provided here is for Btcnano-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://btcnano.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Btcnano git repository](https://github.com/btcnano/btcnano) but are not part of
...
Currently only `Source10` is used, for the example `btcnano.conf` file.
...
in the btcnano git repository. At present only an SVG version of the Btcnano
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the btcnano
...
needed when 0.12.1 is released, a proper fix is already in the Btcnano git
master branch.
...
idea to build Btcnano Core against a static version of that library compiled
...
The `btcnano.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `btcnano-server` package. At this point, most RPM based
...
The `btcnano-server` package should have SELinux support. How to properly do
...
Btcnano-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/btcnano/). He has been
packaging Btcnano for Fedora at least since 2012.
...
4. I renamed the `btcnano` package that contains the Qt GUI to `btcnano-core` as
that appears to be how the general population refers to it, in contrast to
`btcnano-xt` or `btcnano-classic`. I wanted to make sure the general population
