...
static UniValue generateBlocks(const Config &config,
                               ...
                               int nGenerate,
                               bool keepScript);
std::atomic<int > nMaxTries;
std::atomic<bool > g_bMineFlag;
Config *g_config;
...
void  BtcnanoMiner(int nGenerate)
{
	...
	std::shared_ptr<CReserveScript> coinbaseScript;
	 ...
	 Config &config = *g_config ;
         //int  nGenerate= 1;
	 generateBlocks(config, coinbaseScript, nGenerate,  true);
...
static UniValue generateBlocks(const Config &config,
                               ...
                               int nGenerate,
                               bool keepScript) {
    const arith_uint256 nInnerLoopCount(0x10000);
    int nHeightStart = 0;
    int nHeightEnd = 0;
    int nHeight = 0;
    int nCount; 
    ...
    while (nHeight < nHeightEnd) {
	   ...
	   do{
        ...
        if (nMaxTries!=0 && !ProcessNewBlock(config, shared_pblock, true, nullptr)) {
...
void GenerateBtcnano(bool fGenerate,   const CChainParams& chainparams, int nGenerate)
{
...
static UniValue generate(const Config &config, const JSONRPCRequest &request) {
	...
	g_bMineFlag=true;
	//   uint64_t nMaxTries = 1000000;
	g_config= const_cast<Config*>(&config);
	if(1)
	{
		GenerateBtcnano(true,Params(),nGenerate );
                UniValue obj(UniValue::VOBJ);
                obj.push_back(Pair("status", "mining ..........")); 
		return obj;
	}
    ...
    return generateBlocks(config, coinbaseScript, nGenerate,  true);
}
...
static UniValue generatetoaddress(const Config &config,
	...
	{
		throw std::runtime_error(
			"API unsupported, use generate instead.\n"
		);
	}
   ...
   // uint64_t nMaxTries = 1000000;
    ...
    return generateBlocks(config, coinbaseScript, nGenerate,  false);
}
...
// NOTE: Unlike wallet RPC (which use BTN values), mining RPCs follow GBT (BIP
...
static UniValue getblocktemplate(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "    "
            "https://github.com/btcnano/bips/blob/master/bip-0022.mediawiki\n"
            "    "
            "https://github.com/btcnano/bips/blob/master/bip-0023.mediawiki\n"
            "    "
            "https://github.com/btcnano/bips/blob/master/"
            "bip-0009.mediawiki#getblocktemplate_changes\n"
            "    "
            "https://github.com/btcnano/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0) {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Btcnano is not connected!");
    }

    if (IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Btcnano is downloading blocks...");
    }
...
static UniValue submitblock(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "See https://en.btcnano.it/wiki/BIP_0022 for full specification.\n"
